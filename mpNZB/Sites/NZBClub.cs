...
namespace mpNZB.Sites
{
  class NZBClub: iSite
  {
    ...
    public void AddItem(XmlNode Node, GUIListControl lstList)
    {
      ...
      Dialogs.AddItem(lstList, Node["title"].InnerText, strTemp.Substring(intSizePOS, strTemp.IndexOf("in", intSizePOS) - intSizePOS).Replace("GB", " GB").Replace("MB", " MB"), Node["link"].InnerText.Replace("nzbcollection.aspx", "nzbdownload.aspx"), 1);
    }
